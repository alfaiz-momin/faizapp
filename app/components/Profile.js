...
function Profile() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        setState((draft) => {
          draft.profileData = respose.data
        })
      } catch (e) {
  ...
  useEffect(() => {
    if (state.startFollowingRequestCount) {
      ...
      async function fetchData() {
        try {
          const respose = await Axios.post(`/addFollow/${state.profileData.profileUsername}`, { token: appState.user.token })
          setState((draft) => {
  ...
  useEffect(() => {
    if (state.stopFollowingRequestCount) {
      ...
      async function fetchData() {
        try {
          const respose = await Axios.post(`/removeFollow/${state.profileData.profileUsername}`, { token: appState.user.token })
          setState((draft) => {
